...
LOT_SIZE = 75  # Updated to default lot size of 75
...
def get_expiry():
    ...
    if datetime.datetime.now().date() == pd.to_datetime(expiries[0]).date():
        return expiries[1]  # Next week's expiry if today is expiry
    else:
        return expiries[0]
...
def get_otm_straddle():
    ...
    otm_ce_strike = atm_strike + 200
    otm_pe_strike = atm_strike - 200
...
def main():
    ...
    while True:
        ...
        print(f"CE LTP: {ce_ltp}, PE LTP: {pe_ltp}, Combined PnL: {pnl_percent:.2f}%")
        ...
        if not leg_exited:
            if pnl_percent >= 5:
                if ce_pnl < pe_pnl:
                    square_off(ce_sym)
                    leg_held = pe_sym
                    logging.info(f"Target reached. Exiting CE with PnL {ce_pnl:.2f}%")
                else:
                    square_off(pe_sym)
                    leg_held = ce_sym
                    logging.info(f"Target reached. Exiting PE with PnL {pe_pnl:.2f}%")
                leg_exited = True
            ...
            expected_trail = 2.5 + ((max_pnl_percent // 3) * 2)
            ...
            if pnl_percent <= trail_sl_percent:
                logging.info(f"Trailing SL hit ({trail_sl_percent}%). Exiting remaining leg {leg_held}.")
                square_off(leg_held)
                break
