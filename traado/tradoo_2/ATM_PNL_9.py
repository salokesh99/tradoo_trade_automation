...
def get_otm_straddle():
    ...
    ce_strike = atm_strike + 500
    pe_strike = atm_strike - 500
...
def wait_for_crossover():
    logging.info("Waiting until 10:30 AM to check crossover...")
    target_time = datetime.time(10, 30)
    while datetime.datetime.now().time() < target_time:
        time.sleep(1)

    logging.info("Fetching historical candles for crossover check")
    candles = kite.historical_data(
        instrument_token=get_spot_token(),
        from_date=datetime.datetime.now().replace(hour=9, minute=15, second=0, microsecond=0),
        to_date=datetime.datetime.now().replace(hour=10, minute=30, second=0, microsecond=0),
        interval="5minute"
    )
